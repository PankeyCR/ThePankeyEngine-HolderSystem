	...
	namespace pankey{
		...
		TestResult TR_TypeList_Testing_24(){
			...
			TList<CharArray> i_list;

			val<CharArray> i_var_0 = CharArray("hellow");
			val<CharArray> i_var_1 = CharArray("world");

			i_list.add(i_var_0);
			i_list.add(i_var_1);

			val<CharArray> i_var_00 = i_list.get(0);
			val<CharArray> i_var_01 = i_list.get(1);

			result.assertEqual("value 0 should be hellow", i_var_00.getValue(), CharArray("hellow"));
			result.assertEqual("value 1 should be world", i_var_01.getValue(), CharArray("world"));
		...
		void TR_TypeList_Testing(TestRunner& a_test_runner){
			...
			a_test_runner.add("List CharArray", TR_TypeList_Testing_24);
		}	
